...
export const HomeView: React.FC<HomeViewProps> = ({ onNavigate, onOpenQuiz, onSelectTerm }) => (
  <div className="space-y-16 pb-16 bg-[#F7F4EC] text-[#24342D]">
    ...
    <section className="mx-auto max-w-7xl px-6 sm:px-10 lg:px-12"><div className="grid gap-8 rounded-3xl border border-[#3F604F]/20 bg-[#203D32] p-8 text-[#F7F4EC] shadow-xl sm:p-12 lg:grid-cols-[1.2fr_.8fr] lg:items-center"><div><div className="inline-flex items-center gap-2 rounded-full border border-[#C49A3A]/40 bg-[#C49A3A]/10 px-4 py-1.5 text-xs font-semibold text-[#E2C978]"><Sparkles className="h-3.5 w-3.5"/> Exploración guiada</div><h2 className="mt-5 font-display text-4xl leading-tight sm:text-5xl">Un mapa para reconocerte mientras caminas.</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-white/75 sm:text-base">El sitio no pretende decirte quién eres. Abre puertas para que observes, practiques y descubras por experiencia propia qué necesita tu atención.</p></div><div className="flex justify-start lg:justify-end"><button onClick={onOpenQuiz} className="rounded-full bg-[#C49A3A] px-7 py-4 text-sm font-bold text-[#203D32] transition hover:bg-[#D5B55F]">Hacer el test <ArrowRight className="ml-2 inline h-4 w-4"/></button></div></div></section>
